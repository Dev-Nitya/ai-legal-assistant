    ...
    def check_rate_limit(self, request: Request) -> Tuple[bool, Dict]:
        ...
        for limit_rule in limits:
            ...
            if not is_allowed:
                ...
                logger.warning(f"Rate limit exceeded for IP {ip_identifier} on {endpoint}")
                return False, headers
            
        # Check session-based limits for expansive endpoints
        if endpoint.startswith("/api/chat") or endpoint.startswith("/api/enhanced-chat"):
            ...
            for limit_rule in self.config.session_limits:
                ...
                if not is_allowed:
                    ...
                    logger.warning(f"Session rate limit exceeded for {session_identifier} on {endpoint}")
                    return False, headers
        ...
        for limit_rule in self.config.global_limits:
            ...
            if not is_allowed:
                ...
                logger.error(f"Global rate limit exceeded on {endpoint}")
                return False, headers
